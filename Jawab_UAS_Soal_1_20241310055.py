...
def npm_20241310055_preprocessing(text):
    ...
    # 5. Lemmatization (WordNet - for English context)
    lemmatizer = WordNetLemmatizer()
    words = [lemmatizer.lemmatize(w) for w in words]

    # 6. STEMMING BAHASA INDONESIA (PENTING UNTUK NEGATIF)
    try:
        factory = StemmerFactory()
        stemmer = factory.create_stemmer()
        # Kita stem per kata agar lebih cepat
...
def npm_20241310055_main_process():
    ...
    # 3. MODELLING
    print(f"\n[PROSES 3] TRAINING MODEL LOGISTIC REGRESSION (WITH BALANCED CLASS WEIGHT)...")
    X = df['clean_text']
    y = df['label']
    ...
    # Model (DITAMBAHKAN class_weight='balanced' agar model lebih sensitif terhadap label negatif)
